...
| **floorplan** | Three folders, three jobs: instance (the wiring), modules (the workshop), chromes (hands off) — each with its own CLAUDE.md playbook filled with your real paths (backed up + appended, never clobbered). A live checklist sets the layout up after a fresh install; a migration scan + copyable agent brief moves a long-running instance over. |
