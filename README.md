...
| **horse-browser** | The control board for the browser your agents drive — installed and updated from npm (`@pa1nd/horse-browser`, harness vendored). One home for the whole stack: the **enforced Bitwarden credential broker** (a signed local daemon holding the only vault session — secrets typed over CDP, gated by per-collection policy · an origin check · a native macOS approval, never entering the model), the live wall of agent sessions · harness daemons · open tabs, a real compositing probe + DeskPad lid-closed vision, and the rule/manual agents load. macOS only. |
...
| **module-dev** | Set up for serious module development: instance (the wiring), modules (the workshop), chromes (hands off) — the instance and chromes folders get CLAUDE.md playbooks filled with your real paths (backed up + appended, never clobbered). A live checklist sets the layout up after a fresh install; a migration scan + copyable agent brief moves a long-running instance over. |
