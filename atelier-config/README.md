...
| **Tailnet sharing** | Every TCP listener owned by this Atelier's process tree (module sidecars attributed to their module), each individually shareable over Tailscale via `tailscale serve --tcp` — Atelier itself stays loopback-bound. An optional one-shot LaunchAgent re-asserts the forwards after a Tailscale restart. |
| **Cloudflare tunnel** | Where each public hostname resolves — an honest two-leg verdict per hostname (is the cloudflared connector running *on this host*, and is the local origin port bound by *us*), plus install / start / stop / restart / uninstall of the cloudflared LaunchAgent. Tunnel descriptors are per-machine state you declare in `data/tunnels.json` (starts absent; the tab shows how); the tunnel config itself stays in the hand-kept `~/.cloudflared/config.yml`, only read. |
