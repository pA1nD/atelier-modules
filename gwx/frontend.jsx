/* gwx — the whole Google Workspace, one safe multi-account command.
 *
 * Extracted from claude5iq (retired): its gwx chapter, now a standalone module.
 * The page: the suite it reaches (brand tiles) → three features (accounts ·
 * requests · skills, with the searchable 97-skill catalogue modal) → why a
 * CLI + a skill beats an MCP, with the getting-started commands and the live
 * install card (official installer from GitHub; gws version vs npm).
 */

import { Reveal, ChapterIntro, Step, ActionConsole, Card, Icon, Modal, VersionTag, ACCOUNT_COLORS, cn, useChromeStyles, useSnapshot, useActions } from './lib.jsx'

const { useState, useEffect } = React

// meta must be a pure object literal — the shell reads it statically.
export const meta = { chrome: 'catalyst-chrome', icon: 'mails', name: 'gwx' }
...
// a small but real markdown renderer for a skill's SKILL.md (frontmatter stripped):
// headings, code fences (w/ language), blockquotes, ordered + unordered lists,
// tables, rules, and inline code/bold/italic/links.
...
// On phones the two panes take turns: the list fills the modal until a skill is
// picked; the detail then fills it, with a back arrow in its header.
  ...
  return (
    <>
      ...
      <div className="flex h-[70vh] min-h-0">
        ...
        <div className={cn('flex w-[300px] shrink-0 flex-col border-r border-zinc-950/10 dark:border-white/10', sel ? 'max-sm:hidden' : 'max-sm:w-full')}>
        ...
        {/* detail */}
        <div className={cn('min-w-0 flex-1 overflow-auto bg-zinc-50/70 dark:bg-zinc-950/40', !sel && 'max-sm:hidden')}>
          {cur ? (
            <div key={sel} className="cl-fadein">
              <div className="sticky top-0 z-10 border-b border-zinc-950/10 bg-white/90 px-6 py-4 backdrop-blur dark:border-white/10 dark:bg-zinc-900/90">
                <div className="flex flex-wrap items-center gap-2">
                  <button onClick={() => setSel(null)} className="-ml-1 rounded-lg p-1 text-zinc-400 transition hover:bg-zinc-950/[0.06] hover:text-zinc-700 sm:hidden dark:hover:bg-white/10 dark:hover:text-zinc-200"><Icon name="arrow-left" size={16} /></button>
...
// live readout under the intro — accounts and sign-in state, straight from the machine
function StatusStrip({ snap, skillCount }) {
  const gwx = snap?.gwx
  return (
      ...
      {!snap ? (
        ...
        <>
          <span className="inline-flex items-center gap-1.5 text-zinc-500 dark:text-zinc-400">
            ...
            <span className="cl-mono font-medium text-zinc-700 dark:text-zinc-200">gwx</span> installed · {(gwx.accounts || []).length} {(gwx.accounts || []).length === 1 ? 'account' : 'accounts'} · {(gwx.authed || []).length} signed in
          </span>
          {skillCount > 0 && <><span className="text-zinc-300 dark:text-zinc-600">·</span><span className="text-zinc-500 dark:text-zinc-400">{skillCount} skills ready</span></>}
        </>
      ) : (
        <span className="inline-flex items-center gap-1.5 font-semibold text-amber-600 dark:text-amber-400"><span className="size-1.5 rounded-full bg-amber-400" /> gwx not installed — set it up below</span>
      )}
    </div>
  )
}
...
export default function Module() {
  ...
  return (
    <div className="cl-root relative">
      <Reveal>
        <ChapterIntro icon="mails" color={ACCENT} kicker="gwx"
          idea="Your whole Google Workspace, in your agent’s hands."
          why="Email, calendar, files, sheets — most real work lives in Google Workspace, and agents are genuinely good at it once they have a way in. gwx is that way in: one command that reaches every app across all your accounts, plus a library of ready-made agent skills — per-service helpers, multi-step recipes, whole personas — loaded on demand." />
        <StatusStrip snap={snap} skillCount={data?.count || 0} />
      </Reveal>
      ...
      <Reveal className="@container">
        <Step label="What makes it powerful" color={ACCENT} title="Three things at once" className="!mt-20"
          ...
          <div className="mt-2 grid grid-cols-1 gap-4 @3xl:grid-cols-3">
      ...
      <Reveal className="@container">
        <Step label="Why a CLI" color={ACCENT} title="A skill + a CLI beats an MCP" className="!mt-16">
          <div className="grid grid-cols-1 items-start gap-x-12 gap-y-8 @4xl:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
            <div>
            ...
            <Card className="p-5">
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <div className="mt-0.5 text-[12px] leading-relaxed text-zinc-500 dark:text-zinc-400">{installed ? `${accounts.length} account${accounts.length === 1 ? '' : 's'} signed in.` : 'install it to connect your Google accounts — one npm package.'}</div>
                  {installed && (
              ...
              {/* the install command in the open — copy-paste it, or let the button run it.
                  Installing and updating are the same npm command. */}
              <div className="mt-4">
                <p className="mb-1.5 text-[11px] font-medium uppercase tracking-[0.12em] text-zinc-400 dark:text-zinc-500">{installed ? 'installed & updated via' : 'the whole install'}</p>
                <CmdLine cmd="npm install -g @pa1nd/gwx" />
              </div>
              {installed ? (
                ...
                <div className="mt-4 flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-[11px] text-zinc-400 dark:text-zinc-500">runs that command for you</span>
                </div>
              )}
