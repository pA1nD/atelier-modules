...
- **Presence-gate viewer-state watchers.** A backend watcher that only feeds this module's UI must idle when nobody watches: the `GET /snapshot` route stamps `slot.watchedAt`, the frontend re-GETs the snapshot every 45s while the tab is visible (single-flight, 10s abort, fixed cadence — a failure never speeds it up), and the tick starts with `if (!force && Date.now() - (slot.watchedAt || 0) > 90000) return`. The 45s re-GET doubles as the staleness net for WS frames lost across a reconnect. Genuinely useful background work (data collection, supervision, sync) stays ungated.
